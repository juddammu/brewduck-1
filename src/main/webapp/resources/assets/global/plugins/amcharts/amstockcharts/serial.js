    ...
    formatPeriodString: function (a, b) {
        if (b) {
            ...
            for (var h = 0; h < c.length; h++) {
                ...
                a = AmCharts.formatValue(a, {open: p, close: v, high: s, low: r, average: n, sum: k, count: m}, d, l, g + "\\.", this.usePrefixes, this.prefixesOfSmallNumbers, this.prefixesOfBigNumbers);
...
AmCharts.Cuboid = AmCharts.Class({construct: function (a, b, c, d, e, f, l, h, g, k, m, p, v, r, s, n) {
    this.set = a.set();
    this.container = a;
    this.h = Math.round(c);
    this.w = Math.round(b);
    ...
    1 > Math.abs(d) && (d = 0);
    1 > Math.abs(c) && (c = 0);
    !isNaN(n) && (0 < Math.abs(e) || 0 < Math.abs(f)) ? (l = [w], l = {fill: l, stroke: k, "stroke-width": g, "stroke-opacity": m, "fill-opacity": h}, I ? (h = "M0,0 L" + c + "," + (d / 2 - d /
