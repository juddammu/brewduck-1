...
AmCharts.wedge = function (a, b, c, d, f, e, g, h, k, l, m, n) {
    ...
    359.94 < x && (x = 359.94);
    f >= x && (f = x);
    var t = 1 / 180 * Math.PI, x = b + Math.sin(d * t) * h, z = c - Math.cos(d * t) * p, A = b + Math.sin(d * t) * e, w = c - Math.cos(d * t) * g, s = b + Math.sin((d + f) * t) * e, u = c - Math.cos((d + f) * t) * g, q = b + Math.sin((d + f) * t) * h, t = c - Math.cos((d + f) * t) * p, y = {fill: AmCharts.adjustLuminosity(l.fill, -.2), "stroke-opacity": 0, "fill-opacity": l["fill-opacity"]}, B = 0;
    180 < Math.abs(f) && (B = 1);
    d = a.set();
    var G;
...
AmCharts.AmMap = AmCharts.Class({inherits: AmCharts.AmChart, construct: function (a) {
    this.cname = "AmMap";
    this.type = "map";
    this.theme = a;
    this.version = "3.11.1";
    ...
    if (this.sizeChanged && AmCharts.hasSVG && this.chartCreated) {
        ...
        this.chartDiv.style.textAlign = "", this.chartDiv.setAttribute("class", "ammapAlert"), this.chartDiv.innerHTML = this.svgNotSupported, this.fire("failed", {type: "failed", chart: this}), clearInterval(this.interval))
}, invalidateSize: function () {
    ...
    if (AmCharts.isModern && !this.preventHover) {
        ...
        isNaN(b) || isNaN(c) || !(2 < Math.abs(a.x - b) || Math.abs(a.y - c)) || (this.mapWasDragged = !0, a = {type: "dragCompleted", zoomX: this.zoomX(), zoomY: this.zoomY(), zoomLevel: this.zoomLevel(), chart: this}, this.fire(a.type, a));
    ...
    var g = b.customData;
    void 0 == g && (g = "");
    return a = AmCharts.massReplace(a, {"[[title]]": f, "[[value]]": e,
    ...
    var a = this.zoomLevel(), a = {chart: this, type: "writeDevInfo", zoomLevel: a, zoomX: this.zoomX(), zoomY: this.zoomY(), zoomLatitude: this.zoomLatitude(), zoomLongitude: this.zoomLongitude(), latitude: this.coordinateToLatitude((this.mouseY - this.mapContainer.y) / a), longitude: this.coordinateToLongitude((this.mouseX - this.mapContainer.x) /
...
AmCharts.ZoomControl = AmCharts.Class({construct: function (a) {
...
AmCharts.SimpleButton = AmCharts.Class({construct: function () {
...
AmCharts.SmallMap = AmCharts.Class({construct: function (a) {
    this.cname = "SmallMap";
    ...
    minimize: function () {
        ...
        var a = this.chart, b = {fill: this.mapColor, stroke: this.mapColor, "stroke-opacity": 1}, c = a.svgData.g.path, d = this.container, f = d.set(), e;
...
AmCharts.AreasProcessor = AmCharts.Class({construct: function (a) {
    this.chart = a
}, process: function (a) {
    this.updateAllAreas();
    this.allObjects = [];
...
AmCharts.AreasSettings = AmCharts.Class({construct: function (a) {
...
AmCharts.ImagesProcessor = AmCharts.Class({construct: function (a) {
    this.chart = a;
    this.reset()
}, process: function (a) {
    var b = a.images, c;
...
AmCharts.ImagesSettings = AmCharts.Class({construct: function (a) {
...
AmCharts.LinesProcessor = AmCharts.Class({construct: function (a) {
    this.chart = a;
    this.reset()
}, process: function (a) {
    ...
    for (u = 0; u < b.length; u++) {
        ...
        if (q.remainVisible || c.selectedObject == q.parentObject) {
            ...
            if (0 < E.length) {
                ...
                q.fixedSize && N && (this.linesToResize.push({line: N, thickness: B}), this.linesToResize.push({line: G, thickness: 3}));
                q.lineSvg = N;
...
AmCharts.LinesSettings = AmCharts.Class({construct: function (a) {
...
AmCharts.MapObject = AmCharts.Class({construct: function (a) {
...
AmCharts.MapArea = AmCharts.Class({inherits: AmCharts.MapObject, construct: function (a) {
    this.cname = "MapArea";
    AmCharts.MapArea.base.construct.call(this, a);
    AmCharts.applyTheme(this, a, this.cname)
}});
AmCharts.MapLine = AmCharts.Class({inherits: AmCharts.MapObject, construct: function (a) {
    this.cname = "MapLine";
    this.longitudes = [];
    this.latitudes = [];
    this.x = [];
...
AmCharts.MapImage = AmCharts.Class({inherits: AmCharts.MapObject, construct: function (a) {
    this.cname = "MapImage";
    this.scale = 1;
...
AmCharts.MapData = AmCharts.Class({inherits: AmCharts.MapObject, construct: function () {
    this.cname = "MapData";
...
AmCharts.DescriptionWindow = AmCharts.Class({construct: function () {
...
AmCharts.ValueLegend = AmCharts.Class({construct: function (a) {
...
AmCharts.ObjectList = AmCharts.Class({construct: function (a) {
    this.divId = a
}, init: function (a) {
    this.chart = a;
    var b;
