                        ...
                        <form:form class="form-horizontal form-banded form-bordered" id="searchForm"
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-10">
                                    <div class="input-group">
                                        <div>
                                            <button id="spiceButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(2)" value="2"></button>
                                            <button id="flavorButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(3)" value="3"></button>
                                            <button id="waterAgentButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(4)" value="4"></button>
                                            <button id="finingButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(5)" value="5"></button>
                                            <button id="herbButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(6)" value="6"></button>
                                            <button id="otherButton" type="button"
                                                    class="btn btn btn-default btn-sm"
                                                    onclick="setTypeSearch(1)" value="1"></button>
                        ...
                        <div class="table-responsive" data-pattern="priority-columns"
                            ...
                            <table cellspacing="0"
                                ...
                                <c:forEach items="${list}" var="list">
                                    <tr>
                                        <td>
                                            <a href="/misc/${list.id}/${list.titleInUrl}">${list.name}</a>
                                        </td>
                                        <td> ${list.name}</td>
                                        <td> ${list.typeKorean}</td>
                                        <td><span
                                                class="badge badge-success">${list.useForKorean}</span>
                                        </td>
                                    </tr>
                                </c:forEach>
    ...
    <script>
        function getTypeCount() {
            $.get("/misc/countMiscType", function (data, status) {
                $("#spiceButton").html("(" + data.spiceCntName + data.spiceCnt + " 건)");
                /*향신료*/
                $("#flavorButton").html("(" + data.flavorCntName + data.flavorCnt + " 건)");
                /*풍미*/
                $("#waterAgentButton").html("(" + data.waterAgentCntName + data.waterAgentCnt + " 건)");
                /*첨가물*/
                $("#finingButton").html("(" + data.finingCntName + data.finingCnt + " 건)");
                /*청정제*/
                $("#herbButton").html("(" + data.herbCntName + data.herbCnt + " 건)");
                /*허브*/
                $("#otherButton").html("(" + data.otherCntName + data.otherCnt + " 건)");
                /*기타*/
            })
        }
